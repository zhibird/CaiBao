...
1. CLI 必须安装在服务端宿主机；Docker 镜像未打包 Node.js，容器内启用需自行扩展镜像（与 MCP 的容器说明同口径）。`docker-compose.yml` 已透传全部 `CLI_AGENT_*` 环境变量（默认关闭），扩展镜像后在宿主 `.env` 设 `CLI_AGENT_ENABLED=true` 即可，容器内工作区默认落在 `/data/cli_agent_workspaces`。
