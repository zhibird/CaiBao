...
class AgentBridge:
    ...
    async def _handle_message(self, inbound: InboundMessage) -> None:
        """处理一条入站消息：创建 Run → SSE 流式消费 → 汇总出站。

        流式处理策略：
        - llm.delta 事件会累积文本，达到阈值时分段推送
        - tool.* 事件会实时推送工具状态
        - run.completed / step.completed 推送最终答案
        """
        try:
            ...
            # 2. SSE 流式消费，边收边发
            acc_text: list[str] = []  # 累积 LLM 增量文本
            char_count = 0  # 自上次 flush 以来的字符数
            final_answer = ""
            ...
            async for event in self._stream_run(run_id):
                if event.event == "llm.delta":
                    ...
                    if delta:
                        ...
                        # 每累积 _STREAMING_FLUSH_CHARS 个字符推送一次
